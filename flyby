...
class flyby:
    ...
    def fitness(self, x):
        ...
        for i in range(self.dim - 1):
            self.l.append(lambert_problem(self.r[i], self.r[i + 1],
                                          (self.t[i + 1] - self.t[i]) * DAY2SEC,
                                          self.mu_sun, False, self.multi_revs))
        ...
        res = self.f_all
        for i in range(1, self.dim - 1):
            ta = acos(dot(self.vi[i], self.vo[i]) /
                      sqrt(dot(self.vi[i], self.vi[i])) / sqrt(dot(self.vo[i], self.vo[i])))
            alt = (self.planets[i].mu_self / dot(self.vi[i], self.vi[i]) *
                   (1 / sin(ta / 2) - 1)) - self.planets[i].safe_radius
            if alt < 0:
                res = res - alt
    ...
    def plot_trajectory(self):
        ...
        for i in range(self.dim):
            ...
            p = keplerian(epoch(self.t[i], "mjd"),
                          ...
                          self.flight_plan[i])
            plot_planet(p, epoch(self.t[i], "mjd"), units=AU, color=colors[self.flight_plan[i]], axes=ax)
            if i != self.dim - 1:
    ...
    def print_transx(self):
        ...
        print("=" * 70)
        print("MJD:                  %10.8f " % epoch(self.t[0], "mjd").mjd)
        ...
        dha = atan2(self.vo[0][2],
                    sqrt(self.vo[0][0] * self.vo[0][0] + self.vo[0][1] * self.vo[0][1])) * RAD2DEG
        rha = atan2(self.vo[0][1], self.vo[0][0]) * RAD2DEG

        print("------------------GMAT------------------")
        print("GMAT TDBModJulian:    ", epoch(self.t[0], "mjd").mjd - 29999.5)
        ...
        for i in range(1, self.dim - 1):
            ...
            ta = acos(dot(self.vi[i], self.vo[i]) /
                      linalg.norm(self.vi[i]) / linalg.norm(self.vo[i]))
            e = 1 / sin(ta / 2)
            rp = a * (1 - e)
            alt = (rp - rad) / 1000
            ...
            print("------------------GMAT------------------")
            print("GMAT TDBModJulian:    ", epoch(self.t[i], "mjd").mjd - 29999.5)
            print("GMAT RadPer:          ", rp / 1000)

            c3 = dot(self.vi[i], self.vi[i]) / 1000000
            dha = atan2(-self.vi[i][2],
                        sqrt(self.vi[i][0] * self.vi[i][0] + self.vi[i][1] * self.vi[i][1])) * RAD2DEG
            ...
            dha = atan2(self.vo[i][2],
                        sqrt(self.vo[i][0] * self.vo[i][0] + self.vo[i][1] * self.vo[i][1])) * RAD2DEG
        ...
        print("Orbit insertion burn  %10.4f m/s - C3 = 0" %
              round(self.f[-1], 4))

        c3 = dot(self.vi[-1], self.vi[-1]) / 1000000
        dha = atan2(self.vi[-1][2],
                    sqrt(self.vi[-1][0] * self.vi[-1][0] + self.vi[-1][1] * self.vi[-1][1])) * RAD2DEG
        rha = atan2(self.vi[-1][1], self.vi[-1][0]) * RAD2DEG

        print("------------------GMAT------------------")
        print("GMAT TDBModJulian:    ", epoch(self.t[-1], "mjd").mjd - 29999.5)
        print("GMAT IncomingC3:      ", c3)
        print("GMAT IncomingRHA:     ", rha)
        print("GMAT IncomingDHA:     ", dha)
        print("")
